...
const EventEditTickets = ({ 
  ...
  const canAddMoreTickets = () => {
    ...
    // Free/No Ticket events can have up to 1 free ticket, but allow adding other types
  ...
  const switchTicketType = (newType) => {
    let newTicket;
    if (newType === 'Tabs Tickets') {
      newTicket = Object.assign({}, baseTabsTicket);
    ...
    toast.success(`Switched to ${newType === 'Free' ? 'No Tickets' : newType}`);
  };
      ...
      >
        <div style={{ 
          ...
          <span className={styles['tickets-viewer-container']} style={{ marginRight: '15px' }}>
            ...
            {/* Change Ticket Type button */}
            <div
              style={{
                marginTop: '10px',
                padding: '8px 12px',
                border: '1px dashed #F09925',
                borderRadius: '8px',
                cursor: 'pointer',
                display: 'flex',
                alignItems: 'center',
                ...
                gap: '6px',
                background: showTypeSelector ? '#FFF8F0' : 'transparent',
              }}
              onClick={() => setShowTypeSelector(!showTypeSelector)}
            >
              <span style={{ fontFamily: 'Outfit', fontSize: '13px', fontWeight: 500, color: '#F09925' }}>
                Change Ticket Type
              </span>
              <span className="material-symbols-outlined" style={{ fontSize: '16px', color: '#F09925' }}>
                {showTypeSelector ? 'expand_less' : 'swap_horiz'}
              </span>
            </div>
            ...
            {showTypeSelector && (
              <div style={{ marginTop: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                {[
                  { type: 'Tabs Tickets', label: 'Tickets with Tabs', desc: 'Sell tickets through MyTabs', icon: 'confirmation_number' },
                  { type: 'External link', label: 'External Links', desc: 'Link to external platforms', icon: 'open_in_new' },
                  { type: 'Free', label: 'No Tickets', desc: 'Free entry, no ticketing', icon: 'block' },
                ].map((opt) => {
                  const isCurrent = tickets[0]?.option === opt.type;
                  return (
                    <div
                      key={opt.type}
                      onClick={() => !isCurrent && switchTicketType(opt.type)}
                      style={{
                        padding: '10px 12px',
                        border: isCurrent ? '2px solid #00AAD6' : '1px solid #E0E0E0',
                        borderRadius: '8px',
                    ...
                    >
                      <span className="material-symbols-outlined" style={{ fontSize: '20px', color: '#00AAD6' }}>
                        {opt.icon}
                      </span>
                      <div>
                        <div style={{ fontFamily: 'Outfit', fontSize: '13px', fontWeight: 600, color: '#333' }}>
                          {opt.label}
                          {isCurrent && <span style={{ fontSize: '11px', color: '#00AAD6', marginLeft: '6px' }}>(current)</span>}
                        </div>
                        <div style={{ fontFamily: 'Outfit', fontSize: '11px', color: '#666' }}>{opt.desc}</div>
                      </div>
                    </div>
                  );
                })}
