...
export const getToken = async (params) => {
  try {
    const response = await http.post("auth/log-in-web", params);
    return response.data;
  } catch (error) {
...
export const changePassword = async (params) => {
  try {
    ...
    console.log('🔑 changePassword - accessToken present:', !!accessToken, 'length:', accessToken?.length);
    
    const response = await http.post("authMobile/change-password", {
    ...
    console.error('changePassword error:', error.response?.data || error);
