...
import ScratchOffLiveDashboard from "./ScratchOffLiveDashboard";
...
// Mirrors SurveyResultsDashboard.test.jsx mocking style.
    ...
    <MemoryRouter initialEntries={["/admin/my-events/evt-123/experiences/exp-456/live"]}>
      <Routes>
        <Route
          ...
          element={<ScratchOffLiveDashboard />}
        />
      </Routes>
    </MemoryRouter>
  );

// A scratch-offs live-stats payload with two tiers, one sold out.
const mockStatsResponse = (overrides = {}) => ({
  status: 200,
  headers: {},
  data: {
    data: {
      experienceType: "digital-scratch-offs",
      state: "Live",
      analytics: {
        totalCardsIssued: 120,
        totalCardsRevealed: 90,
        totalWins: 12,
        totalNoWins: 78,
        uniqueAttendees: 100,
        tiers: [
...
// A zero-issued payload: tiers present but no cards issued.
...
describe("ScratchOffLiveDashboard", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    jest.useFakeTimers();
  });
  ...
  // 11.1 — renders without crashing and shows per-tier metrics.
  it("renders the title and per-tier metrics", async () => {
    getLiveStats.mockResolvedValue(mockStatsResponse());
    renderComponent();

    await waitFor(() => {
      expect(screen.getByTestId("scratchoff-title")).toBeInTheDocument();
    });
  ...
  // 11.3 — provided metrics render per-tier label, awards, and remaining.
  ...
  // 11.4 — totals cards-issued/cards-revealed/wins/no-wins render.
  it("renders the instance totals", async () => {
    getLiveStats.mockResolvedValue(mockStatsResponse());
    renderComponent();

    await waitFor(() => {
      expect(screen.getByTestId("total-cards-issued")).toHaveTextContent("120");
    });
    expect(screen.getByTestId("total-cards-revealed")).toHaveTextContent("90");
    expect(screen.getByTestId("total-wins")).toHaveTextContent("12");
    expect(screen.getByTestId("total-no-wins")).toHaveTextContent("78");
  });

  // 11.5 — a tier with zero remaining inventory shows a sold-out indicator.
  ...
  // 11.6 — a Closed instance shows final metrics and a closed indicator, stops polling.
  ...
  // 11.8 — zero issued cards shows an empty-state indicator.
  it("shows an empty-state indicator when no cards have been issued", async () => {
    ...
    expect(screen.getByTestId("total-cards-issued")).toHaveTextContent("0");
  ...
  it("renders a default export for lazy loading", async () => {
    const module = await import("./ScratchOffLiveDashboard");
