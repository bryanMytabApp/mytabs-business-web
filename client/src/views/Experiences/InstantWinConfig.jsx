...
/**
 * Validates the instant-win configuration form, reproducing the backend plugin
...
export function validateAll(form) {
  ...
  // Rule 1.8 — plays-per-attendee limit integer 1..1000.
  const playsLimit = Number(form?.playsPerAttendeeLimit);
  if (
    !Number.isInteger(playsLimit) ||
    playsLimit < LIMITS.MIN_PLAYS_LIMIT ||
    playsLimit > LIMITS.MAX_PLAYS_LIMIT
  ) {
    errors["playsPerAttendeeLimit"] =
      `Plays per attendee must be a whole number between ${LIMITS.MIN_PLAYS_LIMIT} and ${LIMITS.MAX_PLAYS_LIMIT}.`;
  }
...
/**
 * InstantWinConfig — organizer-facing configuration screen for an Instant Win
 * experience instance. Mirrors ScratchOffConfig/RaffleConfig's stepped MUI
 * form: an errors map keyed by field path, memoized prize-tier editors,
...
const InstantWinConfig = () => {
      ...
      try {
        ...
        if (!cancelled && cfg?.prizePool?.length) {
          setForm({
            ...
            playsPerAttendeeLimit:
              typeof cfg.playsPerAttendeeLimit === "number" ? cfg.playsPerAttendeeLimit : 1,
            noWinMessage: cfg.noWinMessage ?? "",
          });
        }
      } catch (err) {
        // Non-fatal: a brand-new instance has no config yet.
        console.log("InstantWinConfig: no existing config", err?.message);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const handleSave = async () => {
    ...
    try {
      const config = {
        ...
        playsPerAttendeeLimit: Number(form.playsPerAttendeeLimit),
      ...
      setSaveError(
        err.response?.data?.message || err.message || "Failed to save instant win configuration."
      );
    } finally {
      setSaving(false);
    }
  ...
  const renderPrizePoolStep = () => (
    <Box>
      ...
      <Typography sx={{ fontSize: 13, color: "#6B7280", mb: 2 }}>
        Add one tier per prize. Each tier sets how many prizes are available and the chance a play
        wins that tier.
      </Typography>
      ...
      <ThemeColorPicker
        ...
        helper="Accent color used across this Instant Win's display"
      />
      ...
      {/* Live Total_Win_Probability readout (Requirement 9.5). */}
      <Box
  ...
  const renderRulesStep = () => (
    <Box>
      ...
      <Typography sx={{ fontSize: 13, color: "#6B7280", mb: 2 }}>
        Set how many times each attendee can play and the message shown when a play does not win.
      </Typography>
      ...
      <Box sx={{ display: "flex", gap: 1.5, flexWrap: "wrap", mb: 2 }}>
        <TextField
          size="small"
          type="number"
          label="Plays Per Attendee"
          value={form.playsPerAttendeeLimit ?? ""}
          onChange={(e) => {
            const raw = e.target.value;
            setForm((prev) => ({
              ...prev,
              playsPerAttendeeLimit: raw === "" ? "" : parseInt(raw, 10),
            }));
            setSaved(false);
          }}
          error={!!errors["playsPerAttendeeLimit"]}
          helperText={
            errors["playsPerAttendeeLimit"] ||
            `${LIMITS.MIN_PLAYS_LIMIT}–${LIMITS.MAX_PLAYS_LIMIT} (default 1)`
          }
          inputProps={{ min: LIMITS.MIN_PLAYS_LIMIT, max: LIMITS.MAX_PLAYS_LIMIT, step: 1 }}
          sx={{ width: 220 }}
        />
      </Box>
      ...
      <TextField
        ...
        helperText="Shown when a play does not win a prize."
        sx={{ mb: 3 }}
      />
  ...
  return (
    <EngagementConfigShell
      title="Configure Instant Win"
      subtitle="Build a tiered prize pool and set how many times each attendee can play."
      steps={STEPS}
...
export default InstantWinConfig;
