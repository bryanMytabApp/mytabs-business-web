...
import InstantWinConfig from "./InstantWinConfig";
...
// Mirrors ScratchOffConfig.test.jsx mocking style.
...
// Fill the minimum valid instant-win: one tier with a label, a valid quantity,
// and a valid probability; the default for plays-per-attendee (1) is valid.
...
describe("InstantWinConfig", () => {
  ...
  it("renders the config form in its default state without crashing", () => {
    ...
    expect(screen.getByText("Configure Instant Win")).toBeInTheDocument();
    expect(screen.getByLabelText("Prize Label")).toBeInTheDocument();
  });

  // 9.2 — adding a tier shows label/quantity/win-probability inputs.
  ...
  // 9.3 — add/remove tier controls respect the 1-50 count limit.
  ...
  // 9.4 — the plays-per-attendee-limit and no-win-message controls are present.
  it("shows the plays-per-attendee-limit and no-win-message controls", async () => {
    renderComponent();
    await goToReview();

    expect(screen.getByLabelText("Plays Per Attendee")).toBeInTheDocument();
    expect(screen.getByLabelText("No-Win Message")).toBeInTheDocument();
    // Default plays-per-attendee limit is 1.
    expect(screen.getByLabelText("Plays Per Attendee")).toHaveValue(1);
  });

  // 9.5 — editing probabilities updates the displayed Total_Win_Probability.
  ...
  // 9.6 — saving invalid data shows field errors and does NOT call updateInstance.
  ...
  // 9.7 — saving valid data calls updateInstance (incl. accentColor) and redirects.
  it("saves valid data via updateInstance and redirects to the dashboard", async () => {
    ...
    expect(payload.config.playsPerAttendeeLimit).toBe(1);
  ...
  it("renders a default export for lazy loading", async () => {
    const module = await import("./InstantWinConfig");
